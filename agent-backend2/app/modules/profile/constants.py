...
# Real per-request auth now exists (see app/modules/auth/) — every request
# path resolves its user_id from the caller's JWT via
# app.modules.auth.interface.get_current_user_id, not from this constant.
# TEST_USER_ID survives only as the default value for functions in this
# module (and others) that accept a user_id parameter, and as the identity
# manual test scripts / seed data use — see
# app/modules/profile/schema/seed_test_user.sql for the student.users row
# this needs (student.user_profiles.user_id is a foreign key to it).
