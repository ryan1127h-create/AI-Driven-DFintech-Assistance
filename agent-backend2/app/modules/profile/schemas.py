...
# stored value that predates a constraint being added (or was written before
# validation existed) must still be readable rather than fail to serialize.
# The Literal types are for the write side (ProfilePatch) only, where
# rejecting an invalid value with a 422 is exactly what should happen.
...
class ProfilePatch(BaseModel):
    ...
    # description close to their own wording" (see the extraction prompt in
    # app/modules/profile/agents/resume_agent.py) while still bounding how
    ...
    # app/modules/profile/interface.py::render_profile_summary).
