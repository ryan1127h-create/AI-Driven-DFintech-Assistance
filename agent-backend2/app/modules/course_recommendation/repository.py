...
This module deliberately does NOT open its own database connection: it reads
through the shared app/repositories/knowledge_repository.py (which caches
the full ~184-chunk corpus in process), so the only "repository work" here
is deterministic parsing of chunk content/metadata into typed models. If a
chunk cannot be parsed, it is skipped with a printed warning rather than
silently mis-parsed — the corpus is small and regular, so a parse failure
means the upstream chunking format changed and should be surfaced.
...
from app.modules.course_recommendation.models import CareerRole, Course, CurriculumRule
...
# courses chunk starts with (see rag-data/docs/chunk具体说明.md).
...
_SEGMENT_LABELS = ("Prerequisite:", "Preclusion:", "Skills covered:")
...
# not a second copy of the code/title/faculty already shown elsewhere (the
# per-course prompt line in recommendation_agent.py::_pool_block, and the
# preference-matching haystack in rule_engine.py::_preference_matches).
# Confirmed present in 100% of the 72 live course chunks.
...
# same upstream C-class chunking script, so this mapping is exhaustive.
SKILL_LABEL_TO_ID = {
...
def load_courses() -> list[Course]:
    ...
    if _courses_cache is None:
        parsed = (_parse_course(c) for c in fetch_all_chunks() if c["source_table"] == "courses")
...
def load_curriculum_rules() -> list[CurriculumRule]:
    ...
    (see service.py — the current default is the 2026-08-onwards rule set)."""
    global _rules_cache
