"""
Public interface of the course_recommendation module — the ONLY file other
modules are allowed to import from app.modules.course_recommendation.
Everything else in this package (repository, rule engine, agent, service
internals) is private.

Current consumers:
    - program_comparison::service.py uses resolve_role_skills() for its
      personal match-score calculation.
    - career_planning::service.py uses recommend_courses() so skill-gap and
      course-selection logic isn't duplicated (same implementation, not a
      second calculation).
    - chatbot does not import this module directly — course-related chat
      questions are still answered by plain RAG (see
      app/modules/chatbot/agents/specialists/academic.py). Only
      career_planning and program_comparison's own interfaces are wired
      into chatbot (see their interface.py docstrings).
...
def resolve_role_skills(role_text: str | None) -> dict:
    ...
    the one shared implementation other modules should use.
