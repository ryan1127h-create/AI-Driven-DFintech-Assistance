...
persisted turn log, see repository.py::log_turn), shared by the
...
def run_comparison(state: AgentState) -> tuple[str, list[str], str]:
    ...
    try:
        ...
        answer, sources = render_structured_reply(result, user_message, _COMPARISON_STYLE_PROMPT)
        ...
        answer, hits = run_rag(_legacy_comparison_node.spec, state["messages"])
        return answer, cited_sources(hits), "comparison_agent_fallback"


def comparison_node(state: AgentState) -> dict:
    answer, sources, agent_name = run_comparison(state)
