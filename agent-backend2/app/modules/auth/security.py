"""
Password hashing and JWT issuance/verification — pure functions, no database
access (see app/modules/auth/repository.py for that) and no HTTP concerns
(see app/modules/auth/service.py for how these are wired into
register/login/get_current_user_id).

Uses bcrypt directly rather than passlib — passlib has known compatibility
issues with recent bcrypt releases, and no other module in this project uses
it, so there's no existing convention to match.
"""
