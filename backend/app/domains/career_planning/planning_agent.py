"""
Career-plan writer — the domain's only LLM step.
...
text (from knowledge_retrieval). This agent only writes the narrative —
...
Prompt rules: the career-pathway mapping is advisory guidance compiled by
this project, never an official NUS placement guarantee — always suggest,
...
def write_plan(
    ...
    try:
        content = llm.complete(_SYSTEM_PROMPT, user_prompt, temperature=0.3, max_tokens=_MAX_TOKENS)
