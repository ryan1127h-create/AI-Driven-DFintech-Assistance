"""
Résumé profile agent — a single-purpose extraction step, deliberately not
wired into the chatbot's intent-classifying orchestrator. Invoked directly
and only by service.py, itself called from one dedicated endpoint
(POST /profile/resume). One LLM call, no routing, no tool registration.
"""
...
from app.domains.profile.constants import ACADEMIC_BACKGROUND_VALUES, TARGET_ROLE_VALUES, TECH_LEVEL_VALUES
...
def extract_profile_from_resume(resume_text: str) -> dict:
    """One LLM call. Raises on failure — this runs synchronously as the
    primary action of its own endpoint, so a failure should surface as an
    HTTP error, not be swallowed."""
    # max_tokens is generous relative to the expected JSON output as a
    # safety margin against truncation.
    response_text = llm.complete(_EXTRACTION_PROMPT, resume_text, temperature=0, max_tokens=1500)
