...
service.py for the summarization/read-side business logic built on top of
this. Private to the conversation domain — other domains and the
orchestrator should go through interface.py, never import this module
directly.

Storage shape per session, in the `student` schema:
  - `student.conversations` (hot table, always small): turn_count,
    ...
    see models.py::ConversationState). Read/written every turn — this is
    the ONLY table a normal turn writes to. `conversation_id` doubles as
    the app-level `session_id`.
  - `student.messages` (cold table, one row per conversation, keyed by the
    same conversation_id): archived_blocks is a jsonb array that each
    freeze appends one entry to, and turn_intents is the same shape, moved
    wholesale out of `conversations.pending_turn_intents` for exactly the
    turns that block covers. Both columns are written together, in one
    statement, only by archive_block() — this table is never touched by a
    normal turn, only by a history-block freeze (see
    service.py::freeze_and_unlock). Never read on the per-turn hot path,
    kept for audit.
...
Three backends are available, selected via settings.session_store_backend:
  ...
  "supabase_cached" — durable storage in the `student` schema, with a cache
                       in front as a cache-aside layer.
...
def _preview_from_raw_tail(raw_tail: list) -> str:
    ...
    if there isn't one. Never raises: a malformed entry just gets skipped
    rather than breaking the whole session list."""
    try:
...
class ConversationStore(ABC):
    ...
    def archive_block(
        ...
        entries that cover the same turn range into the archive table, in
        one write. `turn_intents` is the slice of ConversationState.
        pending_turn_intents the caller (service.py::freeze_and_unlock) has
        already selected for this block — this method only persists it, it
        never decides which entries belong here."""
        ...
    ...
    def try_lock(self, session_id: str, status: str, user_id: str) -> str | None:
        ...
        Two distinct lock reasons currently exist:
          - "processing": a turn for this session is being handled — guards
            the whole read-modify-write orchestrator/turn_service.py's
            run_turn() does (via get_state()/record_turn() below) so two
          ...
          - "summarizing": a history-block freeze is in progress (see
            service.py::freeze_and_unlock).
        Returns None if the caller now owns the lock, or the status string
        of whichever lock is currently blocking it otherwise."""
        ...
    ...
    def list_sessions(self, user_id: str) -> list[dict]:
        ...
        raw tail, truncated. Backends with no per-user index (redis)
        return []."""
        ...
...
class RedisConversationStore(ConversationStore):
    ...
    def get_state(self, session_id: str) -> ConversationState:
        raw = cache.get(self._key(session_id))
        if not raw:
            return ConversationState()
        return state_from_json(json.loads(raw))

    def save_state(self, session_id: str, state: ConversationState) -> None:
        cache.set(self._key(session_id), json.dumps(state_to_json(state)), ttl_seconds=settings.session_ttl_seconds)
    ...
    def list_sessions(self, user_id: str) -> list[dict]:
        return []  # no per-user index over cache keys on this backend
    ...
    def try_lock(self, session_id: str, status: str, user_id: str) -> str | None:
        # An atomic NX-SET is the whole point here — only one caller can
        # ever win this, and the TTL is the cache's own enforced expiry, no
        # separate staleness check needed. The lock key's value is the
        # status string itself, so a losing caller can read back what's
        # blocking it without a second lookup racing against the lock
        # expiring in between.
        key = self._lock_key(session_id)
        if cache.set_if_not_exists(key, status, ttl_seconds=settings.freeze_lock_ttl_seconds):
            return None
        ...
        return cache.get(key) or "unknown"

    def release_lock(self, session_id: str) -> None:
        cache.delete(self._lock_key(session_id))
...
class SupabaseConversationStore(ConversationStore):
    """Durable storage in the `student` schema (same conversation database
    every other domain's per-user tables live in) — hot
    ...
    `student.conversations.user_id` is a nullable FK to `auth.users` (same
    identity source every other student.* table's user_id points at —
    see app/domains/auth/repository.py); written as the authenticated
    caller's user_id, threaded down through ConversationState.user_id /
    try_lock()'s user_id argument.
    """
    ...
    def save_state(self, session_id: str, state: ConversationState) -> None:
        conversation_db.execute(
            ...
            (
                ...
                2 * state.turn_count,  # strict Human/AI pairing assumed, same as service.py
            ),
        )
    ...
    def archive_block(
        ...
        only runs from service.py::freeze_and_unlock (never on
        a normal turn), so this table is genuinely archive-only:
        `turn_intents` here is just the slice of the caller's
        ConversationState.pending_turn_intents that this block covers,
        moved out of student.conversations and into this table's own copy
        — the caller is responsible for then dropping those entries from
        ...
        freeze decisions can still each decide to archive the same block,
        so this block's content could still appear twice inside
    ...
    def get_archived_blocks(self, session_id: str) -> list[dict]:
        """archived_blocks is already in append order (oldest block first)
        since archive_block() only ever grows it via jsonb `||`
        concatenation, so no further sorting is needed here (callers may
        still sort defensively by start_turn if they want to be
        independent of that guarantee). archived_blocks and turn_intents
        are two parallel arrays — each archive_block() call appends exactly
        one entry to both in the same statement, so they zip elementwise by
        position."""
        row = conversation_db.fetch_one(
    ...
    def delete(self, session_id: str) -> None:
        ...
        conversation_db.execute("delete from student.conversations where conversation_id = %s", (session_id,))
    ...
    def try_lock(self, session_id: str, status: str, user_id: str) -> str | None:
        ...
        conversation, so the row may not exist yet. A plain `UPDATE ...
        WHERE conversation_id = %s` would silently match zero rows in that
        case (rowcount==0, indistinguishable from "someone else holds a
        live lock" without this upsert) — every caller for a not-yet-
        existing session would then wrongly read that as "acquired" and
        race unprotected. `INSERT ... ON CONFLICT DO UPDATE ... WHERE`
        creates the row (already locked, stamped with the caller's
        user_id) on first use, or conditionally updates it same as before
        — an existing row's user_id is intentionally left untouched (not
        part of the `set` clause), so this can't be used to steal
        ...
        acquired_row = conversation_db.execute_returning(
            """
...
class CachedConversationStore(ConversationStore):
    """Cache-aside wrapper: a cache in front of a durable ConversationStore.

    Caching the whole ConversationState is effective because the state
    stays small forever (bounded raw tail + slowly-growing summaries),
    instead of a blob that grows with total conversation length.
    """
    ...
    def get_archived_blocks(self, session_id: str) -> list[dict]:
        ...
        # conversation's history, not on every turn.
    ...
    def unlock_after_freeze(self, session_id: str, state: ConversationState) -> None:
        ...
        # Refresh the cache immediately so the next get_state() doesn't
        # read the pre-freeze snapshot until the normal TTL would've
        # expired it.
        cache.set(self._key(session_id), json.dumps(state_to_json(state)), ttl_seconds=settings.session_ttl_seconds)
...
def get_session_store() -> ConversationStore:
    """Returns the process-wide conversation store singleton, backend
    chosen by settings.session_store_backend."""
    global _store
    if _store is None:
