...
facts from the shared knowledge base (through the knowledge base adapter,
no domain-owned database of its own).
...
- NUS side: there is no NUS row in competitor_programs, so NUS facts are
  ...
  with what the curriculum rule chunks already cover and, more importantly,
  would push Duration/Workload past _MAX_DIMENSION_CHARS's truncation
  cutoff if included first — page_06 for fees, page_04/05 for admission,
  plus the authoritative test-score FAQ snippet so a superseded GMAT figure
...
from app.domains.specialist.program_comparison.models import ProgramFacts
...
def load_competitors() -> list[ProgramFacts]:
    ...
    if _competitors_cache is None:
        _competitors_cache = [
            _parse_competitor(c)
            for c in knowledge_db.fetch_all_chunks()
