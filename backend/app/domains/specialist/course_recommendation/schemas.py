...
class DegreeProgressResponse(BaseModel):
    core_mandatory_done: list[str]
    core_mandatory_remaining: list[str]
    core_choice_done: list[str]
    core_choice_remaining_count: int
    core_choice_surplus: list[str]
    core_replacement_cautions: list[str]
...
class CourseRecommendationResponse(BaseModel):
    ...
    degree_progress: DegreeProgressResponse | None
    skill_gaps: list[str]
...
def response_from_result(result: RecommendationResult) -> CourseRecommendationResponse:
    ...
    try:
        ...
        return CourseRecommendationResponse(
            ...
            groups=[RecommendationGroup.model_validate(item) for item in result.groups],
            degree_progress=(
                DegreeProgressResponse.model_validate(result.degree_progress.__dict__)
                if result.degree_progress is not None
                else None
            ),
