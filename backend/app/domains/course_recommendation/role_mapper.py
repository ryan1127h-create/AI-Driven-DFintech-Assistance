...
The 6 standard roles (career_roles chunks) come with curated skill lists
and never pass through here. For anything else the user types ("quant
...
def map_role_to_skills(free_text_role: str) -> list[str] | None:
    ...
    try:
        content = llm.complete(_SYSTEM_PROMPT, free_text_role.strip(), temperature=0, max_tokens=_MAX_TOKENS)
