"""Exercises dispatch/evaluation.py's evaluate_branch() directly (not
through the registry — it isn't a registered Tool, see its module
docstring): correct parsing of each action, and that every failure mode
...
from app.orchestrator.dispatch import evaluation as ebt
from app.orchestrator.dispatch.evaluation import EvaluateBranchInput
...
def test_accept_json_returns_default_verdict(monkeypatch):
    ...
    verdict = ebt.evaluate_branch(_input())
...
def test_clarify_action_carries_the_question(monkeypatch):
    ...
    verdict = ebt.evaluate_branch(_input(intent="admissions", user_message="When are deadlines?"))
...
def test_malformed_json_defaults_to_accept(monkeypatch):
    ...
    assert ebt.evaluate_branch(_input()).action == "accept"
...
def test_empty_inputs_skip_the_llm_call_entirely(monkeypatch):
    ...
    assert ebt.evaluate_branch(_input(user_message="")).action == "accept"
    assert ebt.evaluate_branch(_input(draft_text="")).action == "accept"
    assert calls == []
...
def test_the_intent_placeholder_is_filled_into_the_prompt(monkeypatch):
    ...
    ebt.evaluate_branch(_input(intent="career"))
