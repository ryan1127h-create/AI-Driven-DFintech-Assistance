"""Exercises dispatch/synthesis.py's synthesize() directly: drafts are
combined via one streamed LLM call, sources are deduped in first-seen
order across drafts, and agent_used is the "+"-joined list of contributing
agents."""

from __future__ import annotations

from app.orchestrator.dispatch import synthesis as st
from app.orchestrator.dispatch.synthesis import SynthesizeInput
...
def test_synthesize_streams_tokens_through_on_event(monkeypatch):
    ...
    result = st.synthesize(inp, on_event=events.append)
