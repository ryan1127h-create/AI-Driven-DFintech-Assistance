"""
Localization stage — the very last step of every turn: converts the
finished answer into the language the user actually asked in (see
app/tools/turn_context.py::TurnState.reply_language, detected once by
classification.py's intent classifier).

Every generation prompt in this app (RAG specialists, assessment, general
chat, evaluation, synthesis) answers in English, unconditionally, with no
language instruction of its own. That's deliberate, not an oversight:
...
doesn't matter which. dispatch/__init__.py calls this unconditionally,
every turn; "should a conversion actually happen" is decided inside
localize() below, by a plain code check on reply_language — never an LLM's
own judgement call (the same class of prompt-following failure above would
apply just as well to "decide for yourself whether to translate") — so a
request that was already in English costs nothing beyond the no-op check
itself.
...
def localize(inp: LocalizeInput, on_event: OnEvent | None = None) -> ToolAnswer:
    ...
    try:
        ...
        print(f"[localization] Warning: reply-language conversion to {language!r} failed, "
