...
on top of conversation_repository.py. Pairs an LLM call (summarize a
...
redoing the work and racing on whose state update survived. So:
  - should_freeze() is a cheap, synchronous, side-effect-free check — called
    from api.py right after a turn completes, *before* the response is
    sent, together with store.try_lock(session_id, "summarizing") to
...
from app.orchestrator.conversation_models import ConversationState, HistoryBlock
from app.orchestrator.conversation_repository import ConversationStore
...
def freeze_and_unlock(store: ConversationStore, session_id: str) -> None:
    ...
    already won the freeze lock via store.try_lock(session_id,
    "summarizing"). Does the slow part (one LLM call to summarize),
    archives the block (raw messages plus this block's slice of
    pending_turn_intents — this is the only place the archive table ever
    gets written), and always releases the lock via
    store.unlock_after_freeze() — including on failure, so a crashed or
    errored attempt can't wedge the session past
