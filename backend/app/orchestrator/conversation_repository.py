...
conversation_service.py for the summarization business logic built on top
of this.
...
Storage shape per session, in the `student` schema:
  - `student.conversations` (hot table, always small): turn_count,
    ...
    see conversation_models.py::ConversationState). Read/written every
    turn — this is the ONLY table a normal turn writes to.
  ...
  - `student.messages` (cold table, one row per conversation, keyed by the
    ...
    conversation_service.py::freeze_and_unlock). Never read on the per-turn
    hot path, kept for audit.
...
class ConversationStore(ABC):
    ...
    def archive_block(
        ...
        freeze_and_unlock) has already selected for this block — this
        method only persists it, it never decides which entries belong
        here."""
        ...
    ...
    def try_lock(self, session_id: str, status: str, user_id: str) -> str | None:
        ...
        Two distinct lock reasons currently exist:
          - "processing": a turn for this session is being handled — guards
            the whole read-modify-write in turn_service.run_turn() so two
...
class SupabaseConversationStore(ConversationStore):
    ...
    written as the authenticated caller's user_id, threaded down through
    ConversationState.user_id / try_lock()'s user_id argument.
    """
