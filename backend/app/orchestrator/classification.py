"""
Classification — classifies the latest user message into 1-3 of ten intent
...
Seven of the ten categories line up with a registered Tool's
trigger_intents (see tools/registration.py) — "assessment",
"course_recommendation", "general" and "off_topic" are the four
exceptions, each of which must appear alone, never combined with another
intent: assessment and course_recommendation are each their own
registered tool but are never fanned out alongside others (see
FANOUT_INTENTS below) — both are single, self-contained pieces of
personalised computation, not something that composes well with a
separately-answered topic in the same turn; "general" has no tool at
all — it's the plain-conversation fallback this module answers directly,
with no retrieval; "off_topic" also has no tool — dispatch/__init__.py
answers it with a fixed decline, no LLM call at all, since this assistant
exists only to support NUS MSc DFT students and applicants (see the module
docstring's rationale in dispatch/routing.py::route).

This same call also detects reply_language — the language the final reply
should end up in (see orchestrator/dispatch/localization.py, which acts on
it as the very last step of a turn, after every tool/business-logic prompt
below has generated its answer in English as usual). Detecting it here, in
the one call that already reads the latest message, costs nothing extra;
...
orchestrator/dispatch/localization.py's docstring for why the two concerns
are kept apart like this).
"""
...
Classify the user's latest message into one or more of ten categories. Most \
...
"assessment", "course_recommendation", "general", and "off_topic" must NEVER \
appear together with any other intent — if any of the four applies, it must \
...
requirements, unit counts, or semester availability, as a general/informational \
question — NOT a request for a personalised pick. Use this for "what courses \
exist", "what's the capstone about", "what are the elective tracks" — anything \
answerable the same way for any student.

"course_recommendation" — The user wants a PERSONALISED recommendation of which \
specific courses/electives THEY should take, based on their own background, \
completed courses, or career goals — e.g. "which electives should I take", \
"recommend courses for a career in AI", "what should I take next semester", \
"help me plan my course selection". The distinguishing signal vs. "academic" is \
personalisation: the user wants courses picked FOR THEM, not general information \
...
only (used to personalise the "career", "comparison", and \
"course_recommendation" answers — leave them out/null/empty whenever nothing \
is actually said, never guess):
...
or
{"intents": ["course_recommendation"], "target_role_hint": "compliance officer", "program_hints": [], "reply_language": "en"}
or
...
assessment, course_recommendation, general, off_topic)"""

VALID_INTENTS = {
    "admissions", "academic", "financial", "career", "comparison",
    "faq", "assessment", "course_recommendation", "general", "off_topic",
}

# Intents that can be fanned out to in parallel when 2+ are detected
# together. assessment/course_recommendation/general/off_topic are
# deliberately excluded — see prompt above.
...
def classify_intent(
    ...
    and reply_language (see orchestrator/dispatch/localization.py). Returns
    ...
    try:
        ...
        print(f"[classification] Warning: intent classification failed, defaulting to 'general' — {exc}")
        intents, target_role_hint, program_hints = ["general"], None, []
        reply_language = _DEFAULT_REPLY_LANGUAGE

    print(f"[classification] intents classified as: {intents} (reply_language={reply_language})")
    if on_event is not None:
        on_event({
