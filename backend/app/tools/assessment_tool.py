"""
Assessment tool — provides a structured, personalised Application Readiness
Assessment when the user shares their academic/professional background.
Retrieves broad context (no topic filter) so both admission requirements
and programme overview are available. Not built on the plain-RAG factory
(rag_retrieve.py) — this is a bespoke, single-purpose tool with its own
long, highly structured prompt and no fallback path (a raw retrieval
failure already degrades gracefully inside knowledge_retrieval.retrieve()
itself, so there's nothing further to fall back to here).
"""
...
from app.tools.rag_retrieve import ADMISSIONS_OFFICIAL_SOURCES
...
def _handler(state: TurnState, on_event: OnEvent | None = None) -> ToolAnswer:
    ...
    chunks: list[str] = []
    for chunk in llm.stream(system_prompt, to_chat_messages(state.messages), temperature=0.2, max_tokens=4096):
        chunks.append(chunk)
        if on_event is not None:
            on_event({"type": "token", "text": chunk})

    return ToolAnswer(text="".join(chunks), sources=cited_sources(hits), agent_used="assessment_agent")
