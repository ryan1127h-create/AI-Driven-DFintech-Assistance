...
schema half). Now:
  - should_freeze() is a cheap, synchronous, side-effect-free check — called
    ...
    the response is sent, together with store.try_lock_for_freeze() to
...
def freeze_and_unlock(store: ConversationStore, session_id: str) -> None:
    ...
    store.try_lock_for_freeze(). Does the slow part (one Kimi call to
    summarize), archives the block, and always releases the lock via
