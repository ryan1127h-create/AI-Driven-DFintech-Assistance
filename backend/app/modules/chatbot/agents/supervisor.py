...
actual source_table/metadata partitions — see plan Phase 3), so each
RAG-backed node's retrieval is scoped to the data it's meant to answer from.
"""
...
from app.modules.chatbot.agents.rag_agent import run_rag
...
from app.modules.chatbot.agents.specialists.career import career_node
from app.modules.chatbot.agents.specialists.comparison import comparison_node
...
def classify_intent_node(state: AgentState) -> dict:
    ...
    # max_tokens leaves headroom above the tiny JSON output for Kimi's
    # hidden reasoning budget (see app/clients/kimi_client.py).
...
# by make_rag_agent() (see app/modules/chatbot/agents/rag_agent.py) —
# dispatch_node reuses each node's .spec directly via run_rag(), rather than
# duplicating the role_prompt/filter_topics/boost_topics definitions here.
_SPECIALIST_NODES = {
