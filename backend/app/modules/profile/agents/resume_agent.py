...
(POST /api/v1/profile/resume). One Kimi call, no routing.
"""
...
from app.modules.profile.constants import TARGET_ROLE_VALUES, TECH_LEVEL_VALUES
...
- academic_background_std: one of "finance", "cs_computing", "engineering", \
"business", "other" — best-fit category for academic_background_raw
...
def extract_profile_from_resume(resume_text: str) -> dict:
    """One Kimi call. Raises on failure (unlike the old per-turn extraction
    ...
    # max_tokens leaves headroom above the JSON output for Kimi's hidden
