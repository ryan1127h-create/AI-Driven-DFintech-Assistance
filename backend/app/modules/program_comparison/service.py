...
Straight-line flow:
    ...
    3. compute personal match scores        agents/match_scorer.py, pure code
         profile via profile.interface
         role skills via course_recommendation.interface (one shared impl)
    4. LLM rewrites cells + comments        agents/comparison_agent.py
...
from app.modules.course_recommendation.interface import resolve_role_skills
from app.modules.profile.interface import TEST_USER_ID, get_profile
from app.modules.program_comparison import repository
from app.modules.program_comparison.agents import comparison_agent, match_scorer
from app.modules.program_comparison.models import ProgramFacts, ProgramMatch

SUPPORTED_DIMENSIONS = ("curriculum", "fees", "admission")
DEFAULT_DIMENSIONS = list(SUPPORTED_DIMENSIONS)

# The one requestable dimension we have no data for yet (see plan §7).
...
def compare_programs(
    ...
    profile = get_profile(user_id or TEST_USER_ID)
