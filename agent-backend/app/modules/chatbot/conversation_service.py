...
def freeze_and_unlock(store: ConversationStore, session_id: str) -> None:
    ...
    store.try_lock(session_id, "summarizing"). Does the slow part (one LLM
    call to summarize), archives the block (raw messages plus this block's
    slice of pending_turn_intents — this is the only place student.messages
    ever gets written), and always releases the lock via
    ...
    try:
        ...
        if len(state.raw_tail) >= n_msgs:
            ...
            # into student.messages alongside the block; anything for a
