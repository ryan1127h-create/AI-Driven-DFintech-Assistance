...
class ConversationState:
    ...
    # not-yet-archived turns. Lives here (not in student.messages) so that
    # every turn writes only to student.conversations; freeze_and_unlock()
    # (see conversation_service.py) moves the entries covered by a frozen
    # block into student.messages.turn_intents and drops them from here —
    # see app/modules/chatbot/repository.py for the full write-path story.
    ...
    # app/modules/chatbot/service.py::run_turn) — None only for a session
    # that hasn't been used yet (or predates auth).
