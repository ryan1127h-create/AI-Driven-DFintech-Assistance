...
def classify_intent_node(state: AgentState) -> dict:
    ...
    # more short fields than before) as a safety margin against truncation.
    #
...
def dispatch_node(state: AgentState) -> dict:
    ...
    if on_event is not None:
        on_event({"type": "step", "stage": "dispatch_start", "agents": intents})
    ...
    for future in done:
        ...
        try:
            ...
            if on_event is not None:
                on_event({"type": "step", "stage": "branch_done", "agent": intent, "ok": True})
        except Exception as exc:
            ...
            if on_event is not None:
                on_event({"type": "step", "stage": "branch_done", "agent": intent, "ok": False})
    for future in not_done:
        ...
        if on_event is not None:
            on_event({"type": "step", "stage": "branch_done", "agent": intent, "ok": False, "timeout": True})
    ...
    for chunk in synthesis_llm.stream(synthesis_messages):
        if not chunk.content:
            continue
