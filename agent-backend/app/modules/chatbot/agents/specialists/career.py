...
def run_career(
    ...
    answer (the single-intent career_node below). supervisor.py's
    ...
    try:
        ...
        answer, sources = render_structured_reply(result, user_message, _CAREER_STYLE_PROMPT, on_event=on_event)
        ...
        answer, hits = run_rag(_legacy_career_node.spec, state["messages"], on_event=on_event)
...
def career_node(state: AgentState) -> dict:
    ...
    if on_event is not None:
        on_event({"type": "step", "stage": "answering", "agent": "career_agent"})

    answer, sources, agent_name = run_career(state, on_event=on_event)
