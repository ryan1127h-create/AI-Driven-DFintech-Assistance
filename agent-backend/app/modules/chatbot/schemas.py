...
class ConversationTurn(BaseModel):
    ...
    content: str
    # True if this turn came from student.messages.archived_blocks (frozen,
    # permanent) rather than the still-mutable raw tail.
    archived: bool
    # Only set on "ai" turns, and only when this turn's intent-classification
    # log entry is still available (see ConversationState.pending_turn_intents
    # / student.messages.turn_intents) — None for a turn from before this
    # field existed, not for "no intent". Never set on "human" turns.
...
class ConversationHistoryResponse(BaseModel):
    session_id: str
    turn_count: int
    # Same value as student.conversations.last_frozen_end — how many of the
    # leading turns are permanently archived (and therefore not
    # roll-backable, see RollbackRequest below).
