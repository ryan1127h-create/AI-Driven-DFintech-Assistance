...
def run_turn(
    ...
    `on_event`, when given, is threaded into the graph's initial state as
    AgentState["on_event"] and read by individual nodes (classify_intent_node,
    dispatch_node, the RAG/hybrid answer nodes) to report progress ("step")
    and answer-text ("token") events for the streaming /chat/stream endpoint
    (see api.py). Every node treats a missing/None value as "emit nothing",
    so leaving this at its default here (the plain /chat endpoint) changes
    ...
    # Intent-classification log for this turn — lives on ConversationState
    # (persisted to student.conversations by save_state() below, not to
    # student.messages) until a future freeze moves the entries covering
    # this turn into student.messages.turn_intents — see
    # conversation_service.py::freeze_and_unlock and
    # app/modules/chatbot/repository.py's module docstring.
    state.pending_turn_intents = state.pending_turn_intents + [
        {"turn": state.turn_count, "intents": result.get("intents", []), "agent_used": result["agent_used"]}
    ]
...
def rollback_conversation(session_id: str, turns_to_remove: int, user_id: str) -> ConversationState:
    ...
    # Drop the now-rolled-back turns' pending intent-log entries too, so they
    # never get archived into student.messages.turn_intents on a later
    # freeze — these turns no longer exist as far as the conversation goes.
