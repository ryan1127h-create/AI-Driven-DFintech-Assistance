...
Straight-line flow, no branching graph:
    ...
    1. resolve the target role
         ...
         any other text               -> LLM maps it to our 9 skill tags
                                         (agents/role_mapper.py, noted in
                                         the response as AI-inferred)
...
Profile access goes through app.modules.profile.interface only, per the
module-isolation rule.
"""

from __future__ import annotations

from app.modules.course_recommendation import repository
from app.modules.course_recommendation.agents import recommendation_agent, role_mapper, rule_engine
from app.modules.course_recommendation.models import RecommendationResult
...
def recommend_courses(
    ...
    if target_role is None or not completed_courses:
        profile = get_profile(user_id or TEST_USER_ID) or {}
