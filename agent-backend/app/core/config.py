...
class Settings:
    ...
    # LLM call in the app (see app/clients/deepseek_client.py). Unlike the
    # previous provider, DeepSeek accepts the full standard temperature
    # range, so each call site's own temperature is honored as-is.
    deepseek_api_key: str = os.getenv("DEEPSEEK_API_KEY", "")
    deepseek_base_url: str = os.getenv("DEEPSEEK_BASE_URL", "https://api.deepseek.com")
    # deepseek-chat is DeepSeek-V3's non-reasoning endpoint — every call site
    # in this codebase wants a direct answer, not a visible chain-of-thought
    # (that's deepseek-reasoner, a separate model this app doesn't use).
