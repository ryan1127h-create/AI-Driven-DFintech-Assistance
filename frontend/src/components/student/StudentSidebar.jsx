...
export default function StudentSidebar() {
  ...
  const deleteConversation = async (id) => {
    ...
    try {
      ...
      alert(error.message || "Failed to delete the conversation. Please try again.");
      return;
    }

    setConversations((prev) => prev.filter((c) => c.id !== id));

    // If the conversation being deleted is the one currently open in the
    // chat pane, it's now gone server-side too — clear it and go back to a
    // fresh chat rather than leaving the pane showing a deleted session.
    const openSessionId = new URLSearchParams(window.location.search).get("session");
    if (openSessionId === id) {
      clearMessages();
